...
mod tests {
    ...
    fn case9_path() -> std::path::PathBuf {
        test_data_dir().join("case9.m")
    }

    #[allow(clippy::type_complexity)]
    fn setup_case9() -> (Network, Vec<f64>, PtdfRows, HashMap<(usize, usize), f64>) {
        let net = surge_io::matpower::load(case9_path()).unwrap();
        ...
        let ptdf =
            surge_dc::compute_ptdf(&net, &surge_dc::PtdfRequest::for_branches(&all_branches))
                .unwrap();
